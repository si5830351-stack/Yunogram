...
void Step::paintCover(QPainter &p, int top) {
	...
	const auto yunoGramIcon = Ui::PixmapFromImage(YunoAssets::currentAppLogo());
	QIcon(yunoGramIcon).paint(&p, QRect(width() / 2 - yunoGramIcon.width() / 2, planeTop - 16, yunoGramIcon.width(), st::introCoverIcon.height()));
}
