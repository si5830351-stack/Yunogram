	...
	rpl::combine(
		...
		if (item) {
			if (const auto document = item->media()
					? item->media()->document()
					: nullptr) {
				auto musicButton = divider->entity()->add(object_ptr<Ui::SlideWrap<Profile::YunoMusicButton>>(
					divider->entity(),
					object_ptr<Profile::YunoMusicButton>(
						divider->entity(),
				...
				}) | rpl::on_next([=]
										  {
											  ...
											  contextMenu->addAction(
												  settings.adaptiveCoverColor()
													  ? tr::yuno_DisableColorfulCover(tr::now)
													  : tr::yuno_EnableColorfulCover(tr::now),
												  [=]
												  {
													  YunoSettings::getInstance().setAdaptiveCoverColor(!YunoSettings::getInstance().adaptiveCoverColor());
