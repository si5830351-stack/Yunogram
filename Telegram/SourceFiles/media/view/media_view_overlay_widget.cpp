...
bool OverlayWidget::hasCopyMediaRestriction(bool skipPremiumCheck) const {
	...
	// Yunogram: removed; allow downloading any stories
	return false;
}
...
bool OverlayWidget::streamingRequiresControls() const {
	return !_stories
		&& _document;
	// Yunogram: allow vieo messages seeking
	//  && (!_document->isAnimation() || _document->isVideoMessage());
}
...
void OverlayWidget::activate() {
	...
	if (YunoFeatures::StreamerMode::isEnabled()) {
		YunoFeatures::StreamerMode::hideWidgetWindow(_window);
	} else {
		YunoFeatures::StreamerMode::showWidgetWindow(_window);
	}
}
...
void OverlayWidget::clearBeforeHide() {
	YunoState::disableGhostModeOnStoryClose(_storiesSession);
	checkSingleViewMediaBurn();
	_message = nullptr;
	_sharedMedia = nullptr;
	_sharedMediaData = std::nullopt;
