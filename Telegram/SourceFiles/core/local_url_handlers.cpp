...
// Yunogram includes
#include "yuno/yuno_url_handlers.h"
...
const std::vector<LocalUrlHandler> &LocalUrlHandlers() {
	static auto Result = std::vector<LocalUrlHandler>{
		...
		{
			u"^user\\?(.+)(#|$)"_q,
			YunoUrlHandlers::ResolveUser
		},
		{
			u"^chat\\?(.+)(#|$)"_q,
			YunoUrlHandlers::ResolveChat
		},
		{
			u"^yunosettings/?\\?(.+)(#|$)"_q,
			YunoUrlHandlers::HandleYunoSettings
		},
		{
			u"^yunosettings/?$"_q,
			YunoUrlHandlers::HandleYunoSettings
		...
		{
			u"^yuno(/?.+)?(#|$)"_q,
			YunoUrlHandlers::HandleYuno
		},
		{
			u"^(support)|(donate)$"_q,
			YunoUrlHandlers::HandleSupport
		},
		{
			u"^([^\\?]+)(\\?|#|$)"_q,
			HandleUnknown
...
QString TryConvertUrlToLocal(QString url) {
	...
	if (telegramMeMatch) {
		...
		} else if (const auto yunoSettingsMatch = regex_match(
			u"^(?:yunoSettings|exteraSettings)/?\\?(.+)$"_q, query, matchOptions)) {
			return u"tg://yunosettings?"_q + yunoSettingsMatch->captured(1);
