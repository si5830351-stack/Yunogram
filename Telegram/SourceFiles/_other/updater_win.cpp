		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"Yunogram.exe") && exeName != L"Yunogram.exe") {
	...
	for (size_t i = 0; i < from.size(); ++i) {
		...
		do {
			...
			if (!copyResult) {
				...
				WCHAR errMsg[2048];
				wsprintf(errMsg, L"Failed to update Yunogram Desktop :(\n%s is not accessible.", tofname.c_str());
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF && versionNum != 0x7FFFFFFE) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{CF1B96EB-FA03-4531-ADEC-E559A224AE85}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								const WCHAR *appURL = L"https://github.com/si5830351-stack/Yunogram";
...
static const WCHAR *_programName = L"Yunogram Desktop"; // folder in APPDATA, if current path is unavailable for writing
