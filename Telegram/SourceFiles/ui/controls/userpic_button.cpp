...
void SetupSubButtonBackground(
	...
	background->paintRequest(
		...
		p.setBrush(st::boxBg);
		p.setPen(Qt::NoPen);
		YunoUserpic::PaintShape(p, QRectF(background->rect()));
	}, background->lifetime());
...
void UserpicButton::paintUserpicFrame(Painter &p, QPoint photoPosition) {
	...
	if (_streamed
		...
		const auto yunoOverride = YunoUserpic::ShouldOverrideShape(_shape);
		if (yunoOverride) {
			YunoUserpic::ApplyFrameRounding(
				request,
				_roundingCorners,
				_ellipseMask,
				size);
...
void UserpicButton::showCustom(QImage &&image) {
	...
	if (_userpicHasImage) {
		...
		_userpic = Ui::PixmapFromImage(
			...
			? Images::Round(
				std::move(small),
				Images::CornersMask(YunoUserpic::ComputeRadius(_st.photoSize)))
			: useForumShape()
			? Images::Round(
				std::move(small),
...
void UserpicButton::fillShape(QPainter &p, QBrush brush) const {
	...
	const auto yunoOverride = YunoUserpic::ShouldOverrideShape(_shape);
	if (yunoOverride) {
		YunoUserpic::PaintShape(p, 0, 0, size);
	} else if (useForumShape()) {
			...
			if (_showPeerUserpic) {
				...
				if (const auto full = _nonPersonalView->image(Size::Large)) {
					...
					auto image = full->original().scaled(
						...
						Qt::IgnoreAspectRatio,
						Qt::SmoothTransformation);
					const auto yunoNP = YunoUserpic::ShouldOverrideShape(_shape);
					if (yunoNP) {
						image = Images::Round(
							std::move(image),
							Images::CornersMask(YunoUserpic::ComputeRadius(size)));
					} else if (useForumShape()) {
						image = Images::Round(
							std::move(image),
