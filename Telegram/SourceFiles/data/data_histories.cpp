...
void Histories::sendReadRequests() {
	...
	// Yunogram sendReadMessages
	const auto &ghost = YunoSettings::ghost(&_owner->session());
	if (!ghost.sendReadMessages()) {
		DEBUG_LOG(("[Yunogram] Don't read messages"));
		_states.clear();
		return;
	}
	...
	return v::match(message(history, realReplyTo), [&](const auto &request) {
		...
		return sendRequest(history, type, [=](Fn<void()> finish) {
			...
			history->sendRequestId = api->request(
				...
				api->applyUpdates(result, randomId);
				YunoWorker::markAsOnline(&history->owner().session());
