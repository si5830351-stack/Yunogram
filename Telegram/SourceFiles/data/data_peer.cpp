...
QImage PeerData::GenerateUserpicImage(
	...
	if (!radius) {
		...
		if (YunoUserpic::ShouldOverrideShape(shape)) {
			radius = YunoUserpic::ComputeRadius(size);
		}
	}
...
bool PeerData::isYunoNoForwards() const {
	if (const auto user = asUser()) {
		return user->isYunoNoForwards();
	} else if (const auto channel = asChannel()) {
		return channel->isYunoNoForwards();
	} else if (const auto chat = asChat()) {
		return chat->isYunoNoForwards();
	}
	return true;
}
