...
void Controller::openInMediaView(Media::View::OpenRequest &&request) {
	if (request.story()) {
		...
		auto &ghost = YunoSettings::ghost(&story->session());
		...
		if (suggestGhostMode) {
			...
			show(Ui::MakeConfirmBox({
				.text = tr::yuno_SuggestGhostModeStoryText(tr::now, tr::rich),
				.confirmed = [=](Fn<void()> close) {
					close();
					YunoSettings::ghost(&story->session()).setGhostModeEnabled(true);
					YunoState::setDisableGhostModeOnStoryClose(&story->session());
				...
				.confirmText = tr::yuno_SuggestGhostModeStoryActionTextYes(),
				.cancelText = tr::yuno_SuggestGhostModeStoryActionTextNo(),
				.title = tr::yuno_SuggestGhostModeTitle(),
				.strictCancel = true,
			}));
			return;
		}
