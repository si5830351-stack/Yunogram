...
void Filler::addToggleNoForwards() {
	...
	const auto disabledNow = user->isYunoNoForwards();
...
void Filler::fillContextMenuActions() {
	...
	addBanFromChannel();
	addClearHistory();
	YunoUi::AddDeleteOwnMessagesAction(_peer, _topic, _controller, _addAction);
	addDeleteChat();
	addLeaveChat();
	addDeleteTopic();
}
...
void Filler::fillHistoryActions() {
	...
	YunoUi::AddYunogramActions(_peer, _thread, _controller, _addAction);
	addCreateTopic();
	addInfo();
	YunoUi::AddJumpToBeginningAction(_peer, _thread, _controller, _addAction);
	YunoUi::AddOpenChannelAction(_peer, _controller, _addAction);
	addViewAsTopics();
	addManageChat();
	addStoryArchive();
	addSupportInfo();
...
void Filler::fillProfileActions() {
	...
	addToggleTopicClosed();
	YunoUi::AddOpenChannelAction(_peer, _controller, _addAction);
	YunoUi::AddShadowBanAction(_peer, _addAction);
	addViewDiscussion();
	addDirectMessages();
	addExportChat();
	addToggleNoForwards();
...
base::weak_qptr<Ui::BoxContent> ShowForwardMessagesBox(
	...
	state->submit = [=](Api::SendOptions options) {
		...
		const auto yunoForwarding = YunoForward::isYunoForwardNeeded(items) || YunoForward::isFullYunoForwardNeeded(items.front());

		if ((!state->submit || yunoForwarding) && successCallback) {
			successCallback();
		}
		// Yunogram-changed
	};
