...
[[nodiscard]] rpl::producer<TextWithEntities> SetStatusLabel(
	...
	return tr::yuno_YunoPreferences() | rpl::map([](const QString& text) {
		return tr::link(text);
	});
}
...
, _version(AddVersionLabel(_footer)) {
	...
	_telegram->setMarkedText(tr::link(
		u"Yunogram Desktop"_q,
		u"https://github.com/si5830351-stack/Yunogram"_q));
...
void MainMenu::setupSetEmojiStatus() {
	_setEmojiStatus->overrideLinkClickHandler([=] {
		_controller->showSettings(Settings::YunoMain::Id());
	});
}
...
void MainMenu::setupMenu() {
	...
	if (!_controller->session().supportMode()) {
		...
		if (settings.showLReadToggleInDrawer()) {
			addAction(
				tr::yuno_LReadMessages(),
				{&st::yunoLReadMenuIcon}
			)->setClickedCallback([=]() mutable
			{
				auto &ghost = YunoSettings::ghost(&controller->session());
		...
		if (settings.showSReadToggleInDrawer()) {
			...
			addAction(
				tr::yuno_SReadMessages(),
				{&st::yunoSReadMenuIcon}
			)->setClickedCallback([=]
			{
				auto box = Ui::MakeConfirmBox({
					.text = tr::yuno_ReadConfirmationBoxQuestion(),
					.confirmed = callback,
					.confirmText = tr::yuno_ReadConfirmationBoxActionText()
				});
	...
	if (settings.showGhostToggleInDrawer()) {
		auto ghostActiveChanges = YunoSettings::getInstance().useGlobalGhostModeValue()
			| rpl::map([controller = _controller](bool) {
				return YunoSettings::ghost(&controller->session()).ghostModeActiveValue();
			})
			| rpl::flatten_latest();
	...
	if (settings.showStreamerToggleInDrawer()) {
		const auto streamerModeToggle = addAction(
			tr::yuno_StreamerModeToggle(),
			{&st::yunoStreamerModeMenuIcon}
		)->toggleOn(rpl::single(YunoFeatures::StreamerMode::isEnabled()));
		...
		streamerModeToggle->toggledChanges(
			...
			{
				if (enabled) {
					YunoFeatures::StreamerMode::enable();
				} else {
					YunoFeatures::StreamerMode::disable();
				}
			},
