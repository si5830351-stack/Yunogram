...
void CloudList::setup() {
	...
	if (YunoFeatures::MessageShot::isChoosingTheme()) {
		YunoFeatures::MessageShot::resetCustomSelectedEvents() | rpl::on_next([=] {
			_group->setValue(-1);
		}, _outer->lifetime());
	}
...
bool CloudList::applyChangesFrom(std::vector<Data::CloudTheme> &&list) {
	...
	if (YunoFeatures::MessageShot::isChoosingTheme()) {
		if (const auto selected = YunoFeatures::MessageShot::getSelectedFromCustom()) {
...
void CloudList::insert(int index, const Data::CloudTheme &theme) {
	...
	button->addClickHandler([=](Qt::MouseButton button) {
		...
		if (YunoFeatures::MessageShot::isChoosingTheme()) {
			YunoFeatures::MessageShot::setTheme(cloud);
			YunoFeatures::MessageShot::setCustomSelected(cloud);
