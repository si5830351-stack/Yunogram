...
, _context(delegate->elementContext()) {
	...
	const auto deletedOpacityEnabled
		= YunoSettings::getInstance().semiTransparentDeletedMessages();
...
void Element::startDeletedAnimation() {
	if (!YunoSettings::getInstance().semiTransparentDeletedMessages()) {
		_deletedOpacityAnimation.stop();
		_deletedOpacityAnimationTarget = nullptr;
		return;
	}
...
int Element::displayedDateHeight() const {
	if (YunoFeatures::MessageShot::isTakingShot() || isMessageHidden(data())) {
		return 0;
	}
