...
[[nodiscard]] int ComputeRightBadgeWidth(
	...
	if (badge->role == BadgeRole::User) {
		const auto tagWidth = (badge->channel
				&& YunoSettings::getInstance().replaceBottomInfoWithIcons())
...
void Message::refreshRightBadge() {
	...
	if ((hasOutLayout() || context() == Context::WelcomeMessages)
		&& !YunoFeatures::MessageShot::isTakingShot()) {
		if (Has<RightBadge>()) {
			RemoveComponents(RightBadge::Bit());
		}
		return;
	}
	if (YunoFeatures::MessageShot::ignoreRender(
			YunoFeatures::MessageShot::RenderPart::HeaderDecorations)) {
		if (Has<RightBadge>()) {
			RemoveComponents(RightBadge::Bit());
		}
		return;
...
int Message::marginTop() const {
	...
	if (const auto service = Get<ServicePreMessage>()) {
		if (!service->below && !YunoFeatures::MessageShot::isTakingShot()) {
...
void Message::draw(Painter &p, const PaintContext &context) const {
	...
	const auto displayInfo = needInfoDisplay() && !YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Date);
	const auto reactionsInBubble = _reactions && embedReactionsInBubble();
...
BottomRippleMask Message::bottomRippleMask(int buttonHeight) const {
	...
	if (YunoSettings::getInstance().removeMessageTail()) {
