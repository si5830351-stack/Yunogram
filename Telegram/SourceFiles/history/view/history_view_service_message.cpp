...
[[nodiscard]] ServiceTimeMetrics ComputeServiceTimeMetrics(const QString &text) {
	...
	const auto &font = st::yunoServiceTimeBadgeFont;
	result.width = font->width(text)
		+ 2 * st::yunoServiceTimeBadgeHorizontalPadding;
	result.height = font->height + 2 * st::yunoServiceTimeBadgeVerticalPadding;
	result.additionalWidth = std::max(
		st::yunoServiceTimeBadgeGap
			+ result.width
			+ st::yunoServiceTimeBadgeTrailingInset
...
void PaintServiceTime(
	...
	p.setFont(st::yunoServiceTimeBadgeFont);
	p.drawText(
		rect.x() + st::yunoServiceTimeBadgeHorizontalPadding,
		rect.y()
			+ st::yunoServiceTimeBadgeVerticalPadding
			+ st::yunoServiceTimeBadgeFont->ascent,
		text);
}
...
void Service::draw(Painter &p, const PaintContext &context) const {
	...
	if (!onlyMedia) {
		...
		const auto lastLineWidthAdd = (lineWidths.size() > 1
			...
			? st::yunoServiceTimeBadgeLastLineWidthAdd
			: 0;
		...
		if (!timeText.isEmpty() && !lineWidths.empty()) {
			...
			const auto lastTextWidth = qMax(
				...
				0);
			const auto gap = lastTextWidth ? st::yunoServiceTimeBadgeGap : 0;
...
, _text(st::msgMinWidth) {
	if (NeedAboutGroup(_history)) {
		...
		&& YunoSettings::getInstance().disableGreetingSticker()) {
