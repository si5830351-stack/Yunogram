...
void AddMessageActions(
	...
	if (request.item) {
		...
		YunoUi::AddHistoryAction(menu, request.item);
		YunoUi::AddHideMessageAction(menu, request.item);
		YunoUi::AddUserMessagesAction(menu, request.item);
		YunoUi::AddRepeatMessageAction(menu, request.item, context);
		YunoUi::AddMessageDetailsAction(menu, request.item);
	}
	...
	if (request.item) {
		YunoUi::AddReadUntilAction(menu, request.item);
		YunoUi::AddBurnAction(menu, request.item);
	}
}
...
void AddWhoReactedAction(
	...
	if (!YunoUi::needToShowItem(settings.showViewsPanelInContextMenu())) {
		return;
	}
...
void AddSelectRestrictionAction(
		...
		bool addIcon) {
	const auto peer = item->history()->peer;
	if ((!peer->isYunoNoForwards() && !YunoForward::isYunoForwardNeeded(item))
		|| item->isSponsored()) {
		return;
	}
	...
	auto button = base::make_unique_q<Ui::Menu::MultilineAction>(
		...
		((addIcon && !user)
			...
			: st::historyHasCustomEmojiPosition),
		tr::yuno_UnforwardableContextMenuText(
			tr::now,
			tr::rich),
...
bool ItemHasTtl(HistoryItem *item) {
	return false; // Yunogram: allow downloading files with ttl
}
