...
TextState BottomInfo::textState(
	...
	if (!YunoFeatures::MessageShot::isTakingShot() && (_data.flags & (Data::Flag::OutLayout | Data::Flag::Sending))) {
...
void BottomInfo::paint(
	...
	if (!YunoFeatures::MessageShot::isTakingShot() && (_data.flags & Data::Flag::OutLayout)) {
	...
	if (!YunoFeatures::MessageShot::isTakingShot() && (_data.flags & Data::Flag::Sending)
...
void BottomInfo::layoutDateText() {
	...
	if (!settings.replaceBottomInfoWithIcons()) {
		const auto deleted = (_data.flags & Data::Flag::YunoDeleted)
		...
		if (_data.flags & Data::Flag::YunoBurnt) {
		...
		if (_data.flags & Data::Flag::YunoBurnt) {
			...
			if (!(_data.flags & Data::Flag::YunoDeleted)
				&& !(_data.flags & Data::Flag::Edited)) {
				burnt.append(' ');
			}
		}
...
struct BottomInfo::Data BottomInfoDataFromMessage(not_null<Message*> message) {
	...
	if (item->isDeleted()) {
		result.flags |= Flag::YunoDeleted;
	}
	if (item->isBurnt()) {
		result.flags |= Flag::YunoBurnt;
	}
	if (!forwarded) {
		return result;
	}
