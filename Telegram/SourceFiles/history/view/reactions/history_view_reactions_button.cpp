...
void Manager::paintButton(
	...
	if (expanded) {
		...
		const auto radius = YunoUserpic::ComputeRadiusF(_inner.height());
	...
	if (expanded) {
		const auto radiusMin = YunoUserpic::ComputeRadiusF(_inner.height());
		const auto radiusMax = YunoUserpic::ComputeRadiusF(_inner.width());
