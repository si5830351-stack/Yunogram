...
void InlineList::paint(
	...
	if (!animations.empty()
		...
		&& !YunoFeatures::MessageShot::isTakingShot()) { // fix crash when taking shot
...
void InlineList::paintSingleBg(
	...
	if (!areTags()) {
		const auto radius = YunoUserpic::ComputeRadiusF(fill.height());
		p.setBrush(color);
...
InlineListData InlineListDataFromMessage(not_null<Element*> view) {
	...
	if (YunoSettings::getInstance().filtersEnabled()) {
