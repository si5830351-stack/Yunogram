...
void GroupedMedia::draw(Painter &p, const PaintContext &context) const {
	...
	if (_parent->media() == this && (!_parent->hasBubble() || isBubbleBottom())) {
		...
		if (needInfoDisplay() && !YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Date)) {
			_parent->drawInfo(
				p,
				context,
				fullRight,
...
bool GroupedMedia::applyGroup(const DataMediaRange &medias) {
	...
	for (const auto media : medias) {
		if (!media) {
			continue; // Yunogram: fix ebe44780-7c8b-4964-ba31-b747c947254f
		}
