...
void Gif::draw(Painter &p, const PaintContext &context) const {
	...
	const auto paintInCenter = !_sensitiveSpoiler
		&& (radial
			...
			|| ttlCovered);
	if (paintInCenter && !YunoFeatures::MessageShot::isTakingShot()) {
	...
	if (displayMute && !YunoFeatures::MessageShot::isTakingShot()) {
	...
	if (!unwrapped && !skipDrawingSurrounding && !YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Date)) {
		...
		if (isRound && !YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Date)) {
	...
	if (!inWebPage && !skipDrawingSurrounding && !YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Date)) {
...
void Gif::drawCornerStatus(
	...
	const auto shot = YunoFeatures::MessageShot::isTakingShot();
...
bool Gif::needCornerStatusDisplay() const {
	if (YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Date)) {
		return false;
	}
...
void Gif::ensureTranscribeButton() const {
	if (YunoFeatures::MessageShot::isTakingShot()) {
		_transcribe = nullptr;
		return;
	}
