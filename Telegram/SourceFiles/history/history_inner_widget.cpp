...
void HistoryInner::setupSharingDisallowed() {
	...
	if (_peer->isUser()) {
		_sharingDisallowed = rpl::single(false);
	} else {
...
void HistoryInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	const auto addItemActions = [&](
		...
		YunoUi::AddHistoryAction(_menu, item);
		YunoUi::AddHideMessageAction(_menu, item);
		YunoUi::AddUserMessagesAction(_menu, item);
		YunoUi::AddRepeatMessageAction(_menu, item, HistoryView::Context::History);
		YunoUi::AddMessageDetailsAction(_menu, item);
	};
	...
	const auto addSelectMessageAction = [&](not_null<HistoryItem*> item) {
		...
		YunoUi::AddReadUntilAction(_menu, item);
		YunoUi::AddBurnAction(_menu, item);
	};
	...
	if (lnkPhoto || lnkDocument) {
		...
		if (isUponSelected > 0) {
			...
			YunoUi::AddCreateFilterAction(_menu, _controller, item, selectedText.rich.text);
