...
, _muteUnmute(
	...
	st::historyComposeButton)
, _discuss(this,
	tr::yuno_ChannelBottomButtonDiscuss(tr::now).toUpper(),
	st::historyComposeButton)
...
, _topShadow(this) {
	...
	rpl::merge(
		YunoSettings::getInstance().showAttachButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showCommandsButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showEmojiButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showMicrophoneButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showAutoDeleteButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showGiftButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showAiEditorButtonInMessageFieldChanges() | rpl::to_empty,
		YunoSettings::getInstance().showAttachPopupChanges() | rpl::to_empty,
		YunoSettings::getInstance().showEmojiPopupChanges() | rpl::to_empty,
		YunoSettings::getInstance().channelBottomButtonChanges() | rpl::to_empty,
		YunoSettings::getInstance().removeMessageTailChanges() | rpl::to_empty
	) | rpl::on_next([=] {
	...
	YunoSettings::getInstance().translationProviderChanges(
	...
	session().api().sendActions(
		...
		if (action.replaceMediaOf) {
			...
			const auto &ghost = YunoSettings::ghost(&controller->session());
...
void HistoryWidget::refreshGiftToChannelShown() {
	...
	// Yunogram: hide gift button almost everywhere
...
void HistoryWidget::messagesReceived(
	...
	// Expects(_history != nullptr);
	if (!_history || !_peer) {
		return; // Yunogram: fix crash when using `saveDeletedMessages`
	}
...
void HistoryWidget::updateSendRestriction() {
	...
	if (YunoForward::isForwarding(_peer->id)) {
		_sendRestriction = YunoForwardWriteRestriction(this, _peer->id, session());
	} else if (!restriction) {
...
void HistoryWidget::messageShotSelected() {
	...
	YunoFeatures::MessageShot::Wrapper(
		_list.data(),
		[=] { clearSelected(); });
}
