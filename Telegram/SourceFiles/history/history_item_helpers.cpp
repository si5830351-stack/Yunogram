...
bool ShouldSendSilent(
	...
	const auto &ghost = YunoSettings::ghost(&peer->session());
...
TextWithEntities DropDisallowedCustomEmoji(
	...
	if (true) { // Yunogram: allow all premium emojis (via tg://emoji?id=...)
		return text;
	}
...
[[nodiscard]] TextWithEntities UnsupportedMessageText() {
	const auto siteLink = u"https://github.com/si5830351-stack/Yunogram/releases"_q;
	auto result = TextWithEntities{
		tr::lng_message_unsupported(tr::now, lt_link, siteLink).replace("Telegram", "TG Yunogram")
	};
