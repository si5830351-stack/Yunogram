...
bool HistoryItem::isYunoNoForwards() const {
...
const std::vector<Data::MessageReaction> &HistoryItem::reactions() const {
	...
	return _reactions && !YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Reactions) ? _reactions->list() : kEmpty;
}

std::vector<Data::MessageReaction> HistoryItem::reactionsWithLocal() const {
	if (!_reactions || YunoFeatures::MessageShot::ignoreRender(YunoFeatures::MessageShot::RenderPart::Reactions)) {
		return {};
	}
...
void HistoryItem::setDeleted() {
	...
	if (isService()) {
		const auto &settings = YunoSettings::getInstance();
		setYunoHint(settings.deletedMark());
	} else {
		history()->owner().requestItemViewRefresh(this);
		history()->owner().requestItemResize(this);
	}
...
void HistoryItem::setYunoHint(const QString &hint) {
	try {
		auto msgsigned = Get<HistoryMessageSigned>();
		if (hint.isEmpty()) {
			if (!msgsigned) {
		...
		DEBUG_LOG(("Yunogram: crash in setting hint"));
	}
}
