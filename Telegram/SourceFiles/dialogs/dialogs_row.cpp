...
QRect CornerBadgeTTLRect(int photoSize) {
	return YunoUserpic::OnlineBadgeRect(photoSize, st::dialogsTTLBadgeSize);
}
...
void Row::PaintCornerBadgeFrame(
	...
	if (storiesCount) {
		...
		if (peer && (peer->forum() || peer->monoforum())) {
			...
			const auto radius = singleRadius
				? YunoUserpic::ComputeRadiusF(context.st->photoSize)
	...
	const auto badge = YunoUserpic::OnlineBadgeRect(photoSize, size, stroke);
...
void Row::paintUserpic(
	...
	const auto stroke = st::dialogsOnlineBadgeStroke;
	const auto badge = YunoUserpic::OnlineBadgeRect(
		context.st->photoSize,
		size,
		stroke);
	p.setOpacity(
