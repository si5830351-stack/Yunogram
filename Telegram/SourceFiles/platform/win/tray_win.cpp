...
[[nodiscard]] QImage ImageIconWithCounter(
	...
	static auto lastUsedIcon = YunoAssets::currentAppLogoName();

	if (lastUsedIcon != YunoAssets::currentAppLogoName()) {
		lastUsedIcon = YunoAssets::currentAppLogoName();
	...
	QPainter p(&result);
	PainterHighQualityEnabler hq(p); // Yunogram: fix for lq icons
