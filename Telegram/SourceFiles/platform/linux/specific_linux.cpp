...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/com.yunogram.desktop.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1yunogram.desktop._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1yunogram.desktop.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"com.yunogram.desktop._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"com.yunogram.desktop"_q;
	}());
