...
GhostModeAccountSettings *Manager::resolveGhostSettings() const {
	...
	return session ? &YunoSettings::ghost(session) : nullptr;
}
...
void Manager::retranslate() {
	...
	if (_ghostModeMenu) {
		_ghostModeMenu->setTitle(tr::yuno_CategoryGhostMode(tr::now));
	}
	if (_ghostMode) {
		if (const auto ghost = resolveGhostSettings()) {
			_ghostMode->setText(ghost->isGhostModeActive()
				? tr::yuno_DisableGhostMode(tr::now)
				: tr::yuno_EnableGhostMode(tr::now));
		} else {
			_ghostMode->setText(tr::yuno_EnableGhostMode(tr::now));
		}
	}
	if (_readOnInteract) {
		_readOnInteract->setText(tr::yuno_MarkReadAfterAction(tr::now));
	}
	if (_scheduleMessages) {
		_scheduleMessages->setText(tr::yuno_UseScheduledMessages(tr::now));
	}
}
...
void Manager::buildMenu() {
	buildAppleMenu(_menuBar->addMenu(u"Yunogram"_q));
