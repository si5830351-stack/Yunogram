...
void Tray::rebuildMenu() {
...

	{
		auto minimizeText = _textUpdates.events(
			...
			return _activeForTrayIconAction
				...
				: tr::lng_open_from_tray(tr::now).replace("Telegram", "Yunogram");
		});
	...
	if (settings.showGhostToggleInTray()) {
		auto ghostActiveChanges = YunoSettings::getInstance().useGlobalGhostModeValue()
			| rpl::map([](bool) {
				return YunoSettings::ghost().ghostModeActiveValue();
			})
			| rpl::flatten_latest();
		...
		auto turnGhostModeText = rpl::combine(
			...
			return active
				? tr::yuno_DisableGhostModeTray(tr::now)
				: tr::yuno_EnableGhostModeTray(tr::now);
		});
		_tray.addAction(
	...
	if (settings.showStreamerToggleInTray()) {
		auto turnStreamerModeText = _textUpdates.events(
			...
			[=]
			{
				bool streamerModeEnabled = YunoFeatures::StreamerMode::isEnabled();

				return streamerModeEnabled
						   ? tr::yuno_DisableStreamerModeTray(tr::now)
						   : tr::yuno_EnableStreamerModeTray(tr::now);
			});
		_tray.addAction(
			std::move(turnStreamerModeText),
			[=]
			{
				if (YunoFeatures::StreamerMode::isEnabled()) {
	...
	{
		return tr::lng_quit_from_tray(tr::now).replace("Telegram", "Yunogram");
	});
