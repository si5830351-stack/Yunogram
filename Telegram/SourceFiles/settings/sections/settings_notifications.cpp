...
void NotificationsCount::prepareNotificationSampleSmall() {
	...
	sampleImage.fill(st::notificationBg->c);
	{
		...
		YunoUserpic::PaintShape(p, QRectF(style::rtlrect(padding, padding, userpicSize, userpicSize, width)));
...
void NotificationsCount::prepareNotificationSampleLarge() {
	...
	sampleImage.fill(st::notificationBg->c);
	{
		...
		auto notifyTitle = st::msgNameFont->elided(u"Yunogram Desktop"_q, rectForName.width());
...
void NotifyPreview::paint(Painter &p, int x, int y) {
	...
	if (_nameShown) {
		const auto r = YunoUserpic::ComputeRadiusF(userpic.width());
