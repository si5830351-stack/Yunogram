...
void ColorsPalette::show(Type type) {
	...
	const auto messageShotSelected = YunoFeatures::MessageShot::isChoosingTheme()
		? YunoFeatures::MessageShot::getSelectedColorFromDefault()
		: std::optional<QColor>();
...
void SetupDefaultThemes(
	...
	const auto updateMessageShotPalette = [=](const QString &path)
	{
		if (path.isEmpty()) { // for Default theme (otherwise doesn't dispaly name properly)
			...
			YunoFeatures::MessageShot::setPalette(embeddedPalette);
			return;
		}
		if (const auto color = YunoFeatures::MessageShot::getSelectedColorFromDefault()) {
			const auto type = YunoFeatures::MessageShot::getSelectedFromDefault();
			const auto scheme = ranges::find(kSchemesList, type, &Scheme::type);
			if (scheme != end(kSchemesList)) {
				...
				if (Window::Theme::LoadFromFile(path, &instance, nullptr, nullptr, colorizer)) {
					YunoFeatures::MessageShot::setPalette(instance.palette);
					return;
				}
			}
		}
		...
		if (const auto preview = PreviewFromFile(QByteArray(), path, theme)) {
			YunoFeatures::MessageShot::setPalette(preview->instance.palette);
		}
	};

	const auto chosen = [] {
		if (YunoFeatures::MessageShot::isChoosingTheme()) {
			return YunoFeatures::MessageShot::getSelectedFromDefault();
		}
	...
	const auto schemeClicked = [=](
		...
		if (YunoFeatures::MessageShot::isChoosingTheme()) {
			YunoFeatures::MessageShot::setDefaultSelected(scheme.type);
	...
	group->setChangedCallback([=](Type type) {
		if (YunoFeatures::MessageShot::isChoosingTheme()) {
			...
			group->setValue(type);
			YunoFeatures::MessageShot::setDefaultSelected(type);
	...
	if (YunoFeatures::MessageShot::isChoosingTheme()) {
		palette->selected() | rpl::on_next(
			[=](QColor color)
			{
				YunoFeatures::MessageShot::setDefaultSelectedColor(color);
				refreshColorizer(YunoFeatures::MessageShot::getSelectedFromDefault());
		...
		YunoFeatures::MessageShot::resetDefaultSelectedEvents() | rpl::on_next([=]
			{
				refreshColorizer(YunoFeatures::MessageShot::getSelectedFromDefault()); // hide colorizer
