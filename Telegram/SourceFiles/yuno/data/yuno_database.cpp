...
#include "yuno/libs/sqlite/sqlite_orm.h"
	...
	for (int v = currentVersion + 1; v <= kLatestVersion; ++v) {
		if (migrations.contains(v)) {
			try {
				...
				YunoDatabase::moveCurrentDatabase();
...
namespace YunoDatabase {

void moveCurrentDatabase() {
	const auto time = base::unixtime::now();

	if (QFile::exists("./tdata/yunodata.db")) {
		QFile::rename("./tdata/yunodata.db", QString("./tdata/yunodata_%1.db").arg(time));
	}

	if (QFile::exists("./tdata/yunodata.db-shm")) {
		QFile::rename("./tdata/yunodata.db-shm", QString("./tdata/yunodata_%1.db-shm").arg(time));
	}

	if (QFile::exists("./tdata/yunodata.db-wal")) {
		QFile::rename("./tdata/yunodata.db-wal", QString("./tdata/yunodata_%1.db-wal").arg(time));
	}
}
