...
#include "yuno/ui/boxes/donate_info_box.h"
#include "yuno/ui/toasts.h"
#include "yuno/utils/rc_manager.h"
...
const auto regDateBotFallbackUsername = QString("yunogrambot");
	...
	return [=]
	{
		...
		if (isCustomBadge) {
			...
			text = custom.text.isEmpty()
					   ? (isExtera
							  ? tr::yuno_DeveloperPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  tr::rich)
							  : tr::yuno_SupporterPopup(
								  tr::now,
								  lt_item,
								  TextWithEntities{peer->name()},
								  tr::rich))
			...
			text = peer->isUser()
					   ? tr::yuno_DeveloperPopup(
						   ...
						   TextWithEntities{peer->name()},
						   tr::rich)
					   : tr::yuno_OfficialResourcePopup(
						   tr::now,
						   lt_item,
						   TextWithEntities{peer->name()},
						   tr::rich);
		} else if (isSupporter) {
			text = tr::yuno_SupporterPopup(
				tr::now,
				lt_item,
				TextWithEntities{peer->name()},
				tr::rich);
...
void MarkAsReadThread(not_null<Data::Thread*> thread) {
	...
	YunoWorker::markAsOnline(&thread->session());
}
					 ...
					 [=](Fn<void()> finish)
					 {
						 if (const auto channel = history->peer->asChannel()) {
							 return history->session().api().request(MTPchannels_ReadHistory(
								 ...
								 MTP_int(tillId)
							 )).done([=] { YunoWorker::markAsOnline(&history->session()); }).send();
						 }
						 ...
						 )).done([=](const MTPmessages_AffectedMessages &result)
						 {
							 ...
							 YunoWorker::markAsOnline(&history->session());
						 }).fail([=]
						 {
						 }).send();
					 });
...
void markReadAfterAction(not_null<History*> history) {
	const auto &ghost = YunoSettings::ghost(&history->session());
...
QString formatTTL(int time, bool isDoc) {
	if (time == 0x7FFFFFFF) {
		return isDoc ? tr::yuno_OnePlayTTL(tr::now) : tr::yuno_OneViewTTL(tr::now);
	}
...
void processMessageDelete(not_null<HistoryItem*> item) {
	if (!isMessageSavable(item)) {
		...
		item->setDeleted();
		YunoMessages::addDeletedMessage(item);
	}
}
	...
	)).done([=](const MTPmessages_BotResults &result)
	{
		...
		for (const auto &res : v) {
			...
			if (flag == "EXACT" || flag == "INTERPOLATED") {
				if (!isSelf) {
					resultText = tr::yuno_CreationDateUserApproximately(
						tr::now,
						lt_item1,
					...
					resultText = tr::yuno_CreationDateSelfApproximately(
						tr::now,
						lt_item,
						TextWithEntities{formattedDate},
						tr::rich
				...
				if (!isSelf) {
					resultText = tr::yuno_CreationDateUserEarlier(
						tr::now,
						lt_item1,
						TextWithEntities{userName},
						lt_item2,
					...
					resultText = tr::yuno_CreationDateSelfEarlier(
						tr::now,
						lt_item,
						TextWithEntities{formattedDate},
						tr::rich
				...
				if (!isSelf) {
					resultText = tr::yuno_CreationDateUserLater(
						tr::now,
						lt_item1,
						TextWithEntities{userName},
						lt_item2,
					...
					resultText = tr::yuno_CreationDateSelfLater(
						tr::now,
						lt_item,
						TextWithEntities{formattedDate},
						tr::rich
...
void getChannelJoinOrCreateDate(not_null<ChannelData*> channel, Fn<void(TextWithEntities)> callback) {
	...
	if (channel->inviteDate) {
		...
		result = tr::yuno_CreationDateChat(
			tr::now,
			lt_item1,
			TextWithEntities{channel->name()},
			lt_item2,
...
void applyGhostScheduling(
		...
		int delaySeconds) {
	const auto &ghost = YunoSettings::ghost(session);
