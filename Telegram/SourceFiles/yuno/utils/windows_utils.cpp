...
void processLegacy(const QString &iconPath) {
	...
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/Yunogram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/Yunogram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcuts = {
		path + u"Yunogram Desktop/Yunogram.lnk"_q,
		path + u"Yunogram/Yunogram.lnk"_q,
...
void reloadAppIconFromTaskBar() {
	const auto iconPath = YunoAssets::appIcoPath();
