...
namespace YunoFeatures::MessageShot {
...
bool ignoreRender(RenderPart part) {
	...
	const auto &s = YunoSettings::getInstance().messageShotSettings();
	return isTakingShot()
...
void Make(not_null<QWidget*> box, const ShotConfig &config, const Fn<void(QImage&,bool)>& callback) {
	...
	const auto showBackground = YunoSettings::getInstance().messageShotSettings().showBackground();
	auto render = [=, messages = std::move(messages), delegate = std::move(delegate)](bool final)
	{
		...
		for (int i = 0; i < messages.size(); i++) {
			...
			if (YunoSettings::getInstance().messageShotSettings().revealSpoilers()) {
				view->revealSpoilers();
			}
		}
...
std::shared_ptr<Ui::ChatStyle> BuildShotChatStyle(
	...
	const auto &shot = YunoSettings::getInstance().messageShotSettings();
...
void ShowMessageShotBox(
	...
	const YunoFeatures::MessageShot::ShotConfig config = {
		controller,
