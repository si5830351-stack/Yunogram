...
TextWithEntities ChangeSummaryText(const ApplyChanges &changes) {
	...
	if (!changes.newFilters.empty()) {
		AppendChangeSummary(
			result,
			tr::yuno_FiltersSheetNewFilters(
				tr::now,
				lt_count,
	...
	if (!changes.removeFiltersById.empty()) {
		AppendChangeSummary(
			result,
			tr::yuno_FiltersSheetRemovedFilters(
				tr::now,
				lt_count,
	...
	if (!changes.filtersOverrides.empty()) {
		AppendChangeSummary(
			result,
			tr::yuno_FiltersSheetUpdatedFilters(
				tr::now,
				lt_count,
	...
	if (!changes.newExclusions.empty()) {
		AppendChangeSummary(
			result,
			tr::yuno_FiltersSheetNewExclusions(
				tr::now,
				lt_count,
	...
	if (!changes.removeExclusions.empty()) {
		AppendChangeSummary(
			result,
			tr::yuno_FiltersSheetRemovedExclusions(
				tr::now,
				lt_count,
	...
	if (!changes.peersToBeResolved.empty()) {
		AppendChangeSummary(
			result,
			tr::yuno_FiltersSheetDialogsToResolve(
				tr::now,
				lt_count,
...
void FilterUtils::importFromLink(const QString &link) {
	if (link.isEmpty()) {
		Ui::Toast::Show(tr::yuno_FiltersToastFailFetch(tr::now));
		return;
	}
	...
	connect(
		...
		[=]
		{
			...
			if (jsonString.isNull()) {
				...
				Ui::Toast::Show(tr::yuno_FiltersToastFailImport(tr::now));
...
void FilterUtils::publishFilters() {
	...
	titlePart.setBody("TG Yunogram Filters");
	...
	connect(
		...
		[=]
		{
			...
			if (error == QNetworkReply::NoError && location.isValid()) {
				...
				Ui::Toast::Show(tr::yuno_FiltersToastFailPublish(tr::now));
			}
			reply->deleteLater();
		});
}
...
void FilterUtils::importFromJson(const QByteArray &json) {
	...
	if (!HasChanges(changes)) {
		Ui::Toast::Show(tr::yuno_FiltersToastFailNoChanges(tr::now));
	...
	auto box = Ui::MakeConfirmBox({
		...
		.confirmed = [=](Fn<void()> close) {
			...
			try {
				...
				Ui::Toast::Show(tr::yuno_FiltersToastSuccess(tr::now));
			} catch (...) {
				LOG(("FilterUtils: Failed to apply import changes"));
				Ui::Toast::Show(tr::yuno_FiltersToastFailImport(tr::now));
			}
		},
		.confirmText = tr::yuno_FiltersMenuImport(),
		.title = tr::yuno_FiltersSheetTitle(),
	});
...
QString FilterUtils::exportFilters() {
	...
	const auto excl = YunoDatabase::getAllFiltersExclusions();
...
ApplyChanges FilterUtils::prepareChanges(const QJsonObject &root) {
	...
	const auto existingFilters = YunoDatabase::getAllRegexFilters();
	const auto existingExclusions = YunoDatabase::getAllFiltersExclusions();
...
void FilterUtils::applyChanges(const ApplyChanges &changes) {
	if (!changes.newFilters.empty()) {
		for (const auto &filter : changes.newFilters) {
			YunoDatabase::addRegexFilter(filter);
		}
	}

	if (!changes.removeFiltersById.empty()) {
		for (const auto &id : changes.removeFiltersById) {
			YunoDatabase::deleteExclusionsByFilterId(id);
			YunoDatabase::deleteFilter(id);
		}
	}

	if (!changes.filtersOverrides.empty()) {
		for (const auto &filter : changes.filtersOverrides) {
			YunoDatabase::updateRegexFilter(filter);
		}
	}

	if (!changes.newExclusions.empty()) {
		for (const auto &exclusion : changes.newExclusions) {
			YunoDatabase::addRegexExclusion(exclusion);
		}
	}

	if (!changes.removeExclusions.empty()) {
		for (const auto &exclusion : changes.removeExclusions) {
			YunoDatabase::deleteExclusion(exclusion.dialogId, exclusion.filterId);
		}
	}
