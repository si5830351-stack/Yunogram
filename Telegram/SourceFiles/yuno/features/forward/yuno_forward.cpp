...
#include "apiwrap.h"
#include "lang_auto.h"
#include "yuno/features/forward/yuno_sync.h"
...
std::pair<QString, QString> stateName(const PeerId &id) {
	...
	QString messagesString = tr::yuno_YunoForwardStatusSentCount(tr::now,
															   lt_count1,
	...
	QString chunkString = tr::yuno_YunoForwardStatusChunkCount(tr::now,
															 lt_count1,
	...
	if (state->state == ForwardState::State::Preparing) {
		status = tr::yuno_YunoForwardStatusPreparing(tr::now);
	} else if (state->state == ForwardState::State::Downloading) {
		return std::make_pair(tr::yuno_YunoForwardStatusLoadingMedia(tr::now), "");
	} else if (state->state == ForwardState::State::Sending) {
		status = tr::yuno_YunoForwardStatusForwarding(tr::now);
	} else {
		// ForwardState::State::Finished
		status = tr::yuno_YunoForwardStatusFinished(tr::now);
	}
...
static Ui::PreparedList prepareMedia(not_null<Main::Session*> session,
	...
	const auto prepare = [&](not_null<Data::Media*> media)
	{
		...
		auto prepared = Ui::PreparedFile(YunoSync::filePath(session, media));
...
void sendMedia(
	...
	if (const auto document = primaryMedia->document(); document && document->sticker()) {
		YunoSync::sendStickerSync(session, std::move(message), document);
		return;
	}
	...
	if (mediaType == SendMediaType::Round || mediaType == SendMediaType::Audio) {
		...
		if (!failed && data.size()) {
			file.close();
			YunoSync::sendVoiceSync(session,
								   data,
...
bool isYunoForwardNeeded(const std::vector<not_null<HistoryItem*>> &items) {
	const auto needYunoForward = [&](const auto &item)
	{
		return isYunoForwardNeeded(item);
	};
	return std::ranges::any_of(items, needYunoForward);
}

bool isYunoForwardNeeded(not_null<HistoryItem*> item) {
	if (item->isDeleted() || item->isYunoNoForwards() || item->unsupportedTTL() || (item->media() && item->media()->ttlSeconds())) {
		return true;
	}
	return false;
}

bool isFullYunoForwardNeeded(not_null<HistoryItem*> item) {
	return item->from()->isYunoNoForwards() || item->history()->peer->isYunoNoForwards();
}

struct ForwardChunk
{
	bool isYunoForwardNeeded;
...
void intelligentForward(
	...
	auto currentChunk = ForwardChunk({
		.isYunoForwardNeeded = isYunoForwardNeeded(items[0]),
		.items = currentArray
	});

	for (const auto &item : items) {
		if (isYunoForwardNeeded(item) != currentChunk.isYunoForwardNeeded) {
			...
			currentChunk = ForwardChunk({
				.isYunoForwardNeeded = isYunoForwardNeeded(item),
				.items = currentArray
			});
		}
	...
	for (const auto &chunk : chunks) {
		if (chunk.isYunoForwardNeeded) {
			...
			YunoSync::forwardMessagesSync(session, chunk.items, action, draft.options);
...
void forwardMessages(
	...
	if (!toBeDownloaded.empty()) {
		...
		YunoSync::loadDocuments(session, toBeDownloaded);
	}
	...
	for (int i = 0; i < items.size(); i++) {
		...
		if (!mediaDownloadable(item->media())) {
			YunoSync::sendMessageSync(session, std::move(message));
...
} // namespace YunoFeatures::YunoForward
