...
bool ResolveUser(
	...
	searchUserById(
		...
		[=](const QString &title, PeerData *data)
		{
			...
			Ui::show(Ui::MakeInformBox(tr::yuno_UserNotFoundMessage()));
		}
	);
...
struct ResolvedSetting {
	QString controlId;
	::Settings::Type section = ::Settings::YunoMain::Id();
};
...
bool HandleYunoSettings(
	...
	if (settingName.isEmpty()) {
		controller->showSettings(::Settings::YunoMain::Id());
	} else {
		const auto resolved = ResolveSetting(
			u"yuno/"_q + settingName,
			&controller->session());
