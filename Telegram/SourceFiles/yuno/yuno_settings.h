...
#include "yuno/libs/json.hpp"
#include "yuno/libs/json_ext.hpp"
...
private:
	friend class YunoSettings;

	rpl::variable<bool> _sendReadMessages = false;
	...
	rpl::variable<bool> _ghostModeActive = true;
...
class YunoSettings {
public:
	YunoSettings(const YunoSettings &) = delete;
	YunoSettings &operator=(const YunoSettings &) = delete;
	YunoSettings(YunoSettings &&) noexcept = default;
	YunoSettings &operator=(YunoSettings &&) noexcept = default;

	[[nodiscard]] static YunoSettings &getInstance();
	...
	friend void to_json(nlohmann::json &j, const YunoSettings &s);
	friend void from_json(const nlohmann::json &j, YunoSettings &s);
