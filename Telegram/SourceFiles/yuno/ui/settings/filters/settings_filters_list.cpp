...
#include "yuno/features/filters/filters_utils.h"
...
rpl::producer<QString> YunoFiltersList::title() {
	if (shadowBan) {
		return tr::yuno_FiltersShadowBan();
	}
	if (!dialogId.has_value()) {
		return tr::yuno_RegexFiltersShared();
	}
	...
	if (from) {
		...
		res = tr::yuno_RegexFiltersHeader(tr::now) + " (" + QString::number(did) + ")";
	}

	return rpl::single(res);
}

YunoFiltersList::YunoFiltersList(
	QWidget *parent,
...
void YunoFiltersList::checkBeforeClose(Fn<void()> close) {
	_controller->showExclude = true;
	_controller->shadowBan = false;
	close();
}

void YunoFiltersList::addNewFilter(const RegexFilter &filter, bool exclusion) {
	...
	auto defaultClickHandler = [=, dialogId = dialogId]() mutable
	{
		...
		_contextMenu->addAction(
			...
			[=]
			{
				...
				YunoDatabase::updateRegexFilter(*state);
		...
		_contextMenu->addAction(
			...
			[=]
			{
				YunoDatabase::deleteFilter(state->id);
				YunoDatabase::deleteExclusionsByFilterId(state->id);
				FiltersCacheController::rebuildCache();
				FiltersCacheController::fireUpdate();
			},
			&st::menuIconDelete);
	...
	auto exclusionsClickHandler = [=, controller = _controller, dialogId = dialogId]() mutable
	{
				...
				└── class Ui::ScrollArea
					└── class QWidget
						└── class Ui::PaddingWrap<class Ui::RpWidget>
							└── class Settings::YunoFiltersList
		 */
		...
		YunoDatabase::addRegexExclusion(newExclusion);
		FiltersCacheController::rebuildCache();
		FiltersCacheController::fireUpdate();
	...
	auto deleteExclusionsClickHandler = [=, this]() mutable
	{
		...
		_contextMenu->addAction(
			...
			[=, this]
			{
				...
				YunoDatabase::deleteExclusion(dialogId.value(), state->id);
				FiltersCacheController::rebuildCache();
				FiltersCacheController::fireUpdate();
			},
			&st::menuIconDelete);
...
void YunoFiltersList::initializeSharedFilters(
	not_null<Ui::VerticalLayout*> container) {
	if (dialogId.has_value() && _controller->showExclude.has_value() && _controller->showExclude.value()) {
		filters = YunoDatabase::getByDialogId(dialogId.value());
		exclusions = YunoDatabase::getExcludedByDialogId(dialogId.value());
	} else {
		filters = YunoDatabase::getShared();

		// remove shared filters that already excluded for that peer exclusion
		if (dialogId.has_value() && _controller->showExclude.has_value() && !_controller->showExclude.value()) {
			const auto excludedForDialogId = YunoDatabase::getExcludedByDialogId(dialogId.value());
	...
	if (!filters.empty()) {
		AddSkip(container);
		filtersTitle = AddSubsectionTitle(container, tr::yuno_RegexFiltersHeader());
	...
	if (!exclusions.empty()) {
		...
		excludedTitle = AddSubsectionTitle(container, tr::yuno_RegexFiltersExcluded());
	...
	if (filters.empty() && exclusions.empty()) {
		Ui::AddDividerText(container, tr::yuno_RegexFiltersListEmpty());
	}
}

void YunoFiltersList::initializeShadowBan(not_null<Ui::VerticalLayout*> container) {
	...
	if (YunoSettings::getInstance().shadowBanIds().size() > 0) {
		AddSkip(container);
...
void YunoFiltersList::setupContent(not_null<Window::SessionController*> controller) {
	if (shadowBan) {
