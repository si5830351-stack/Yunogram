...
void RegexEditBuilder(
	...
	if (filter) {
		box->setTitle(showToast ? tr::yuno_RegexFiltersAdd() : tr::yuno_RegexFiltersEdit());
		data = *filter;
	} else {
		box->setTitle(tr::yuno_RegexFiltersAdd());
		data.enabled = true;
	...
	const auto caseInsensitive = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::yuno_CaseInsensitiveExpression(tr::now),
	...
	const auto reversed = box->addRow(
		object_ptr<Ui::Checkbox>(
			box,
			tr::yuno_ReversedExpression(tr::now),
			data.reversed,
			st::defaultBoxCheckbox),
		st::settingsCheckboxPadding);
	...
	auto saveAndClose = [=, id = data.id]
	{
		...
		crl::async([=]
		{
			YunoDatabase::addRegexFilter(newFilter);
			...
			crl::on_main([=]
			{
				...
				if (showToast) {
					auto config = Ui::Toast::Config{
						.text = tr::yuno_RegexFilterBulletinText(
							tr::now,
							tr::rich),
						.adaptive = true,
					};
					if (dialogId.has_value()) {
						Yuno::Ui::ShowToastWithAction(
							std::move(config),
							tr::yuno_RegexFilterBulletinAction(tr::now),
							[=]() mutable {
								newFilter.dialogId = dialogId;

								YunoDatabase::updateRegexFilter(newFilter);
								FiltersCacheController::rebuildCache();
								FiltersCacheController::fireUpdate();
							});
					} else {
