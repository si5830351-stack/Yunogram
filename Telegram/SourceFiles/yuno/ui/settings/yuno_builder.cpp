...
YunoSectionBuilder::YunoSectionBuilder(Builder::SectionBuilder &builder)
: _builder(builder) {
}

Builder::SectionBuilder &YunoSectionBuilder::base() {
	return _builder;
}

Ui::SettingsButton *YunoSectionBuilder::addSettingToggle(
		SettingToggleArgs &&args) {
	auto getter = [g = args.getter] {
		return (YunoSettings::getInstance().*g)();
	};
	auto setter = [s = args.setter](bool v) {
		(YunoSettings::getInstance().*s)(v);
	};
	return addToggle({
		.id = std::move(args.id),
...
Ui::SettingsButton *YunoSectionBuilder::addToggle(ToggleArgs &&args) {
...
Fn<void()> YunoSectionBuilder::addCollapsibleToggle(
...
void YunoSectionBuilder::addChooseButton(ChooseButtonArgs &&args) {
...
void YunoSectionBuilder::addSlider(SliderArgs &&args) {
	auto id = std::move(args.id);
	auto altIds = std::move(args.altIds);
	auto keywords = std::move(args.keywords);
	auto resolvedTitle = ResolveTitle(rpl::duplicate(args.title));
...
void YunoSectionBuilder::addBetaBadge(not_null<Ui::SettingsButton*> button) {
	AddBetaBadge(button);
}

void YunoSectionBuilder::addSectionDivider() {
	_builder.addSkip();
	_builder.addDivider();
	_builder.addSkip();
}
