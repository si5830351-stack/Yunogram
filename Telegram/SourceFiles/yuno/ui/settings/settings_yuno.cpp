...
private:
	void paint(Painter &&p) {
		...
		}
		{
			...
			p.drawImage(
				userpic.inner,
				st::yunoGhostModeGlobalIcon.instance(st::historyPeerUserpicFg->c));
		}
		if (_active) {
			PaintAccountOutline(p, userpic.outer);
		}
...
QString PickerLabel(uint64 userId) {
	return (userId == 0)
		? tr::yuno_GhostModeGlobalSettings(tr::now)
...
void selectGhostProfile(GhostPickerState *state, uint64 userId) {
	...
	YunoSettings::getInstance().setUseGlobalGhostMode(nowGlobal);
	...
	if (wasGlobal != nowGlobal) {
		Ui::Toast::Show(nowGlobal
			? tr::yuno_GhostModeSwitchedToGlobalSettings(tr::now)
			: tr::yuno_GhostModeSwitchedToIndividualSettings(tr::now));
	}
}
...
void BuildGhostEssentials(SectionBuilder &builder) {
	builder.add([](const BuildContext &ctx) {
		v::match(ctx, [&](const WidgetContext &wctx) {
			...
			if (activeCount <= 1 && !YunoSettings::getInstance().useGlobalGhostMode()) {
				auto userId = controller->session().userId().bare;
				auto &src = YunoSettings::ghost(userId);
				auto &dst = YunoSettings::ghost(0);
				...
				YunoSettings::getInstance().setUseGlobalGhostMode(true);
			}

			const auto isGlobal = YunoSettings::getInstance().useGlobalGhostMode();
			...
			const auto title = AddSubsectionTitle(container, tr::yuno_GhostEssentialsHeader());
			...
			std::vector checkboxes{
				NestedEntry{
					tr::yuno_DontReadMessages(tr::now),
					[state] { return !YunoSettings::ghost(state->selectedUserId.current()).sendReadMessages(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendReadMessages(!v); },
					[state] { return YunoSettings::ghost(state->selectedUserId.current()).sendReadMessagesLocked(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendReadMessagesLocked(v); }
				},
				NestedEntry{
					tr::yuno_DontReadStories(tr::now),
					[state] { return !YunoSettings::ghost(state->selectedUserId.current()).sendReadStories(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendReadStories(!v); },
					[state] { return YunoSettings::ghost(state->selectedUserId.current()).sendReadStoriesLocked(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendReadStoriesLocked(v); }
				},
				NestedEntry{
					tr::yuno_DontSendOnlinePackets(tr::now),
					[state] { return !YunoSettings::ghost(state->selectedUserId.current()).sendOnlinePackets(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendOnlinePackets(!v); },
					[state] { return YunoSettings::ghost(state->selectedUserId.current()).sendOnlinePacketsLocked(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendOnlinePacketsLocked(v); }
				},
				NestedEntry{
					tr::yuno_DontSendUploadProgress(tr::now),
					[state] { return !YunoSettings::ghost(state->selectedUserId.current()).sendUploadProgress(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendUploadProgress(!v); },
					[state] { return YunoSettings::ghost(state->selectedUserId.current()).sendUploadProgressLocked(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendUploadProgressLocked(v); }
				},
				NestedEntry{
					tr::yuno_SendOfflinePacketAfterOnline(tr::now),
					[state] { return YunoSettings::ghost(state->selectedUserId.current()).sendOfflinePacketAfterOnline(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendOfflinePacketAfterOnline(v); },
					[state] { return YunoSettings::ghost(state->selectedUserId.current()).sendOfflinePacketAfterOnlineLocked(); },
					[state](bool v) { YunoSettings::ghost(state->selectedUserId.current()).setSendOfflinePacketAfterOnlineLocked(v); }
				},
			};

			auto collapsible = AddCollapsibleToggle(
				container,
				tr::yuno_GhostModeToggle(),
				std::move(checkboxes),
				true,
				tr::yuno_GhostModeOptionShiftDescription(tr::now));
			...
			if (wctx.highlights && collapsible.widget) {
				wctx.highlights->push_back(std::make_pair(
					u"yuno/ghostModeToggle"_q,
					HighlightEntry{ collapsible.widget, {} }));
			}

			const auto markReadButton = AddButtonWithIcon(
				container,
				tr::yuno_MarkReadAfterAction(),
				st::settingsButtonNoIcon
			);
			if (wctx.highlights) {
				wctx.highlights->push_back(std::make_pair(
					u"yuno/markReadAfterAction"_q,
					HighlightEntry{ markReadButton.get(), {} }));
			}
			markReadButton->toggleOn(
				state->selectedUserId.value()
				| rpl::map([](uint64 id) {
					return YunoSettings::ghost(id).markReadAfterActionValue();
				}) | rpl::flatten_latest()
			)->toggledValue(
			) | rpl::filter(
				[=](bool enabled) {
					return enabled != YunoSettings::ghost(state->selectedUserId.current()).markReadAfterAction();
				}
			) | on_next(
				[=](bool enabled) {
					auto &ghost = YunoSettings::ghost(state->selectedUserId.current());
				...
				container->lifetime());
			AddSkip(container);
			AddDividerText(container, tr::yuno_MarkReadAfterActionDescription());

			AddSkip(container);
			const auto scheduleButton = AddButtonWithIcon(
				container,
				tr::yuno_UseScheduledMessages(),
				st::settingsButtonNoIcon
			);
			if (wctx.highlights) {
				wctx.highlights->push_back(std::make_pair(
					u"yuno/useScheduledMessages"_q,
					HighlightEntry{ scheduleButton.get(), {} }));
			}
			scheduleButton->toggleOn(
				state->selectedUserId.value()
				| rpl::map([](uint64 id) {
					return YunoSettings::ghost(id).useScheduledMessagesValue();
				}) | rpl::flatten_latest()
			)->toggledValue(
			) | rpl::filter(
				[=](bool enabled) {
					return enabled != YunoSettings::ghost(state->selectedUserId.current()).useScheduledMessages();
				}
			) | on_next(
				[=](bool enabled) {
					auto &ghost = YunoSettings::ghost(state->selectedUserId.current());
				...
				container->lifetime());
			AddSkip(container);
			AddDividerText(container, tr::yuno_UseScheduledMessagesDescription());

			AddSkip(container);
			const auto silentOptions = std::vector<QString>{
				tr::yuno_SendWithoutSoundByDefaultNever(tr::now),
				tr::yuno_SendWithoutSoundByDefaultInGhostMode(tr::now),
				tr::yuno_SendWithoutSoundByDefaultAlways(tr::now),
			};
			const auto silentOptionText = state->selectedUserId.value(
			) | rpl::map([=](uint64 id) {
				return YunoSettings::ghost(id).sendWithoutSoundValue(
			...
			const auto silentButton = AddButtonWithLabel(
				container,
				tr::yuno_SendWithoutSoundByDefault(),
				std::move(silentOptionText),
				st::settingsButtonNoIcon);
			if (wctx.highlights) {
				wctx.highlights->push_back(std::make_pair(
					u"yuno/sendWithoutSound"_q,
					HighlightEntry{ silentButton.get(), {} }));
			}
			silentButton->addClickHandler([=] {
				controller->show(Box([=](not_null<Ui::GenericBox*> box) {
					const auto save = [=](int index) {
						YunoSettings::ghost(state->selectedUserId.current()
						).setSendWithoutSound(
							static_cast<SendWithoutSoundOption>(index));
					};
					SingleChoiceBox(box, {
						.title = tr::yuno_SendWithoutSoundByDefault(),
			...
			AddSkip(container);
			AddDividerText(container, tr::yuno_SendWithoutSoundByDefaultDescription());

			AddSkip(container);
			const auto suggestGhostModeButton = AddButtonWithIcon(
				container,
				tr::yuno_SuggestGhostModeBeforeViewingStory(),
				st::settingsButtonNoIcon);
			if (wctx.highlights) {
				wctx.highlights->push_back(std::make_pair(
					u"yuno/suggestGhostModeBeforeViewingStory"_q,
					HighlightEntry{ suggestGhostModeButton.get(), {} }));
			}
			suggestGhostModeButton->toggleOn(
				state->selectedUserId.value()
				| rpl::map([](uint64 id) {
					return YunoSettings::ghost(id).suggestGhostModeBeforeViewingStoryValue();
				}) | rpl::flatten_latest()
			)->toggledValue(
			) | rpl::filter(
				[=](bool enabled) {
					return enabled != YunoSettings::ghost(state->selectedUserId.current()).suggestGhostModeBeforeViewingStory();
				}
			) | on_next(
				[=](bool enabled) {
					YunoSettings::ghost(state->selectedUserId.current()).setSuggestGhostModeBeforeViewingStory(enabled);
				},
				container->lifetime());
			AddSkip(container);
			AddDividerText(container, tr::yuno_SuggestGhostModeBeforeViewingStoryDescription());
			...
			sctx.entries->push_back({
				.id = u"yuno/ghostModeToggle"_q,
				.title = tr::yuno_GhostModeToggle(tr::now),
				.section = sctx.sectionId,
			});
			sctx.entries->push_back({
				.id = u"yuno/markReadAfterAction"_q,
				.title = tr::yuno_MarkReadAfterAction(tr::now),
				.section = sctx.sectionId,
			});
			sctx.entries->push_back({
				.id = u"yuno/useScheduledMessages"_q,
				.title = tr::yuno_UseScheduledMessages(tr::now),
				.section = sctx.sectionId,
			});
			sctx.entries->push_back({
				.id = u"yuno/sendWithoutSound"_q,
				.title = tr::yuno_SendWithoutSoundByDefault(tr::now),
				.section = sctx.sectionId,
			});
			sctx.entries->push_back({
				.id = u"yuno/suggestGhostModeBeforeViewingStory"_q,
				.title = tr::yuno_SuggestGhostModeBeforeViewingStory(tr::now),
				.section = sctx.sectionId,
			});
		});
	});
...
void BuildSpyEssentials(SectionBuilder &builder, YunoSectionBuilder &yuno) {
	builder.addSubsectionTitle(tr::yuno_SpyEssentialsHeader());

	yuno.addSettingToggle({
		.id = u"yuno/saveDeletedMessages"_q,
		.title = tr::yuno_SaveDeletedMessages(),
		.getter = &YunoSettings::saveDeletedMessages,
		.setter = &YunoSettings::setSaveDeletedMessages,
	});
	yuno.addSettingToggle({
		.id = u"yuno/saveMessagesHistory"_q,
		.title = tr::yuno_SaveMessagesHistory(),
		.getter = &YunoSettings::saveMessagesHistory,
		.setter = &YunoSettings::setSaveMessagesHistory,
	});

	yuno.addSectionDivider();

	yuno.addSettingToggle({
		.id = u"yuno/saveForBots"_q,
		.title = tr::yuno_MessageSavingSaveForBots(),
		.getter = &YunoSettings::saveForBots,
		.setter = &YunoSettings::setSaveForBots,
	});
}

void BuildOther(SectionBuilder &builder, YunoSectionBuilder &yuno) {
	builder.addSubsectionTitle(tr::yuno_MessageSavingOtherHeader());

	yuno.addSettingToggle({
		.id = u"yuno/localPremium"_q,
		.title = tr::yuno_LocalPremium(),
		.getter = &YunoSettings::localPremium,
		.setter = &YunoSettings::setLocalPremium,
	});
	yuno.addSettingToggle({
		.id = u"yuno/disableAds"_q,
		.title = tr::yuno_DisableAds(),
		.getter = &YunoSettings::disableAds,
		.setter = &YunoSettings::setDisableAds,
	});
}
...
const auto kMeta = BuildHelper({
	...
	builder.addSkip();
	BuildSpyEssentials(builder, yuno);

	yuno.addSectionDivider();
	BuildOther(builder, yuno);
	builder.addSkip();
});

} // namespace

rpl::producer<QString> YunoGhost::title() {
	return rpl::single(QString("TG Yunogram"));
}
