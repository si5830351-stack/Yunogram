...
const QVector<QString> icons{
	YunoAssets::DEFAULT_ICON,
	YunoAssets::ALT_ICON,
	YunoAssets::DISCORD_ICON,
	YunoAssets::SPOTIFY_ICON,
	YunoAssets::EXTERA_ICON,
	YunoAssets::NOTHING_ICON,
	YunoAssets::BARD_ICON,
	YunoAssets::YAPLUS_ICON,
	YunoAssets::WIN95_ICON,
	YunoAssets::CHIBI_ICON,
	YunoAssets::CHIBI2_ICON,
	YunoAssets::EXTERA2_ICON,
};
...
void applyIcon() {
	...
	Window::OverrideApplicationIcon(YunoAssets::currentAppLogo());
	...
	for (int row = 0; row < rows; row++) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (const auto cached = _cachedIcons.find(iconName); cached != _cachedIcons.end()) {
				...
				icon = _cachedIcons[iconName] = YunoAssets::loadPreview(iconName);
			}
			auto opacity = 0.0f;
			if (iconName == _wasSelected) {
				opacity = 1.0f - _animation.value(1.0f);
			} else if (iconName == YunoAssets::currentAppLogoName()) {
		...
		for (int i = 0; i < columns; i++) {
			...
			if (e->pos().x() >= x && e->pos().x() <= x + iconSize
				...
				if (settings.appIcon() != iconName) {
					...
					YunoSettings::getInstance().setAppIcon(iconName);
					changed = true;
					break;
				}
			}
