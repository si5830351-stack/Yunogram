...
#include "yuno/ui/components/saved_music.h"

#include "yuno/yuno_settings.h"
#include "yuno/ui/utils/color_utils.h"
#include "yuno/ui/utils/itunes_search.h"
#include "yuno/ui/utils/palette.h"
...
QRgb AdjustHsl(QRgb color, float luminance, float saturation = -1.0f) {
	auto hsl = Yuno::Ui::ColorUtils::colorToHSL(color);
	...
	return Yuno::Ui::ColorUtils::HSLToRGB(hsl);
}
...
QPixmap MakeNoCoverImage(const QSize &size) {
	...
	auto image = QImage(size, QImage::Format_ARGB32);
	{
		...
		auto svgIcon = QSvgRenderer(u":/gui/icons/yuno/nocover.svg"_q);
...
std::optional<QRgb> ExtractColorFromCover(const QPixmap &cover) {
	const auto palette = Yuno::Ui::Palette::from(cover).generate();
	...
	const auto contrast = Yuno::Ui::ColorUtils::calculateContrast(whiteColor, extractedColor);
	...
	if (Yuno::Ui::ColorUtils::calculateContrast(whiteColor, adjustedColor) < 3.0f) {
...
YunoMusicButton::YunoMusicButton(
	QWidget *parent,
...
YunoMusicButton::~YunoMusicButton() = default;

void YunoMusicButton::updateData(MusicButtonData data) {
...
void YunoMusicButton::downloadAndMakeCover(FullMsgId msgId) {
...
void YunoMusicButton::makeCover() {
	...
	crl::async(
		[=, mediaView = _mediaView, performerText = _performerText, titleText = _titleText, overrideBg = _overrideBg]()
		{
			...
			if (cover.noCover) {
				const auto pix = Yuno::Ui::Itunes::FetchCover(performerText, titleText, size);
				if (!pix.isNull()) {
			...
			crl::on_main([weak, cover = std::move(cover), bgColor, overrideBg]() mutable
			{
				...
				const auto &settings2 = YunoSettings::getInstance();
...
void YunoMusicButton::paintEvent(QPaintEvent *e) {
	if (!_currentCover) {
		return;
	}
...
int YunoMusicButton::resizeGetHeight(int newWidth) {
