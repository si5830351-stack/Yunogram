...
, _state(lifetime().make_state<State>()) {
	_state->bubbleRadius = YunoSettings::getInstance().messageBubbleRadius();
	...
	const auto yunogramUser = HistoryView::GenerateUser(
		history,
	...
	rpl::merge(
		YunoSettings::getInstance().replaceBottomInfoWithIconsChanges()
			| rpl::to_empty,
		YunoSettings::getInstance().deletedMarkChanges()
			| rpl::to_empty,
		YunoSettings::getInstance().editedMarkChanges()
			| rpl::to_empty,
		YunoSettings::getInstance().removeMessageTailChanges()
			| rpl::to_empty,
		YunoSettings::getInstance().hideFastShareChanges()
			| rpl::to_empty,
		YunoSettings::getInstance().simpleQuotesAndRepliesChanges()
			| rpl::to_empty,
		YunoSettings::getInstance().semiTransparentDeletedMessagesChanges()
			| rpl::to_empty
	) | rpl::on_next([=] {
		refresh();
	}, lifetime());
...
void MessagePreview::paintEvent(QPaintEvent *e) {
	...
	if (!YunoSettings::getInstance().hideFastShare()) {
