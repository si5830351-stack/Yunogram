...
void loadAppIco() {
	...
	f.close();
	QFile::copy(qsl(":/gui/art/yuno/%1/app_icon.ico").arg(settings.appIcon()), iconPath);
}
...
QImage CreateImage(const QString &name, const QSize resultImageSize, const int padding = 0) {
	...
	const auto pngPath = qsl(":/gui/art/yuno/%1/app.png").arg(name);
	...
	const auto svgPath = qsl(":/gui/art/yuno/%1/app.svg").arg(name);
