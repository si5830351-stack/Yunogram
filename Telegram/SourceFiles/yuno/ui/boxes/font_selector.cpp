...
void Content::setupContent(
	...
	const auto current = YunoSettings::getInstance().monoFont();
...
YunoUi::FontSelectorBox::FontSelectorBox(QWidget *, Window::SessionController *controller, Fn<void(QString font)> hook)
	: _controller(controller), _hook(hook) {
}

void YunoUi::FontSelectorBox::prepare() {
	...
	addLeftButton(tr::yuno_BoxActionReset(),
				  [=]
				  {
					  _hook(qs(""));
	...
	setTitle(tr::yuno_CustomizeFontTitle());
	...
	const auto fonts = YunoUi::PrepareFonts();
	const auto inner = setInnerWidget(
		object_ptr<YunoUi::Content>(this, fonts),
		st::boxScroll,
		topContainer->height());
...
void YunoUi::FontSelectorBox::setupTop(not_null<Ui::VerticalLayout*> container) {
	if (!_controller) {
		return;
	}
}

void YunoUi::FontSelectorBox::keyPressEvent(QKeyEvent *e) {
...
int YunoUi::FontSelectorBox::rowsInPage() const {
	return std::max(height() / YunoUi::Rows::DefaultRowHeight(), 1);
}

void YunoUi::FontSelectorBox::setInnerFocus() {
	_setInnerFocus();
}

base::binary_guard
YunoUi::FontSelectorBox::Show(Window::SessionController *controller, const Fn<void(QString font)> hook) {
