...
#include "lang_auto.h"
#include "yuno/features/message_shot/message_shot_theme_state.h"
...
void ThemeSelectorBox::setupContent() {
	...
	setTitle(tr::yuno_MessageShotThemeSelectTitle());
	...
	YunoFeatures::MessageShot::themeChosen(
	) | rpl::on_next(
	...
	YunoFeatures::MessageShot::paletteChosen(
	) | rpl::on_next([=](const auto &palette)
							 {
								 ...
								 const auto name = (type != Window::Theme::EmbeddedType(-1))
									 ? YunoFeatures::MessageShot::embeddedThemeDisplayName(type)
									 : tr::yuno_MessageShotThemeDefault(tr::now);
								 _themeNames.fire(QString(name));
								 _selectedPalette = palette;
							 },
							 lifetime());

	addButton(tr::yuno_MessageShotThemeApply(),
			  [=]
			  {
