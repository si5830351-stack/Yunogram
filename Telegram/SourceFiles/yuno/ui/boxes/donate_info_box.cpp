...
QImage MakeSupportLogo() {
	...
	auto svg = QSvgRenderer(QString(":/gui/icons/yuno/donates/support_logo.svg"));
	auto image = QImage(
...
void FillDonateInfoBox(not_null<Ui::GenericBox*> box, not_null<Window::SessionController*> controller) {
	...
	box->verticalLayout()->add(
		object_ptr<Ui::FlatLabel>(
			box->verticalLayout(),
			tr::yuno_SupportBoxHeader(tr::bold),
			st::boxTitle),
		st::boxRowPadding,
		style::al_top);
	...
	const auto str = tr::yuno_SupportBoxMakeDonationInfo(
		tr::now,
		lt_amount1,
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::yuno_SupportBoxMakeDonationHeader(tr::now),
		str,
		&st::menuIconEarn,
		emojiHelper.context()));
	...
	const TextWithEntities proofText = tr::yuno_SupportBoxSendProofInfo(
		tr::now,
		lt_item,
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::yuno_SupportBoxSendProofHeader(tr::now),
		proofText,
		&st::menuIconPhoto,
		Core::TextContext({
	...
	box->verticalLayout()->add(InfoRow(
		box->verticalLayout(),
		&controller->session(),
		tr::yuno_SupportBoxReceiveBadgeHeader(tr::now),
		TextWithEntities{
			tr::yuno_SupportBoxReceiveBadgeInfo(tr::now)
		},
