...
void FillImportFiltersBox(not_null<Ui::GenericBox*> box, bool import) {
	...
	box->setTitle(import ? tr::yuno_FiltersMenuImport() : tr::yuno_FiltersMenuExport());
	...
	addOption(false, import ? tr::yuno_FiltersImportClipboard(tr::now) : tr::yuno_FiltersExportClipboard(tr::now));
	addOption(true, import ? tr::yuno_FiltersImportURL(tr::now) : tr::yuno_FiltersExportURL(tr::now));
	...
	const auto actionButton = box->addButton(
		import ? tr::yuno_FiltersMenuImport() : tr::yuno_FiltersMenuExport(),
		[=]
		{
