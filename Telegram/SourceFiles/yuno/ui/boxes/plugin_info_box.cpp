...
#include "yuno/ui/boxes/plugin_info_box.h"
...
void FillPluginInfoBox(
...

	{
		...
		const auto versionPrefix = tr::yuno_PluginVersion(tr::now)
			+ u" "_q
			+ metadata.version;
...

	{
		...
		auto descText = hasDescription
			...
			: TextWithEntities{tr::yuno_PluginNoDescription(tr::now)};
		if (hasDescription) {
