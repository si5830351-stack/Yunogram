...
#include "yuno/ui/context_menu/context_menu.h"
...
#include "yuno/data/messages_storage.h"
#include "yuno/features/filters/filters_controller.h"
#include "yuno/features/forward/yuno_forward.h"
#include "yuno/ui/context_menu/menu_item_subtext.h"
#include "yuno/ui/message_history/history_section.h"
#include "yuno/ui/settings/filters/edit_filter.h"
#include "yuno/ui/settings/filters/settings_filters_list.h"
#include "yuno/utils/qt_key_modifiers_extended.h"
...
Fn<void()> ClearDeletedMessagesHandler(not_null<Window::SessionController*> controller, not_null<PeerData*> peer, ID topicId) {
	return [=] {
		controller->show(Ui::MakeConfirmBox({
			.text = tr::yuno_ClearDeletedMessagesText(tr::now),
			.confirmed = [=](Fn<void()> &&close) {
				...
				YunoMessages::clearDeletedMessages(peer, topicId);
				for (const auto item : items) {
					item->destroy();
				}
				close();
			},
			.confirmText = tr::yuno_ClearDeletedMessagesActionText(tr::now),
...
Fn<void()> DeleteMyMessagesHandler(not_null<Window::SessionController*> controller, not_null<PeerData*> peer) {
	return [=]
	{
		if (!controller->showFrozenError()) {
			controller->show(Ui::MakeConfirmBox({
				.text = tr::yuno_DeleteOwnMessagesConfirmation(tr::now),
				.confirmed =
				[=](Fn<void()> &&close)
				{
...
void AddYunogramActions(PeerData *peerData,
							   Data::Thread *thread,
	...
	addCallback(Window::PeerMenuCallback::Args{
		...
		.fillSubmenu = [=](not_null<Ui::PopupMenu*> menu) {
			...
			if (showFilters) {
				addAction(
					tr::yuno_ViewFiltersMenuText(tr::now),
					[=]
					{
						...
						sessionController->showSettings(Settings::YunoFiltersList::Id());
					},
			...
			if (filteredToggleShown) {
				addAction(
					*filteredToggleShown
						? tr::yuno_HideFilteredMessagesMenuText(tr::now)
						: tr::yuno_ShowFilteredMessagesMenuText(tr::now),
					[=]
					{
			...
			if (saveDeletedMessages) {
				addAction(
					tr::yuno_ViewDeletedMenuText(tr::now),
					[=]
					{
				...
				addAction({
					.text = tr::yuno_ClearDeletedMenuText(tr::now),
...
void AddJumpToBeginningAction(PeerData *peerData,
	...
	addCallback(
		tr::yuno_JumpToBeginning(tr::now),
		[=]
		{
			if (user) {
...
void AddShadowBanAction(PeerData *peerData,
	...
	const auto shadowBanned = YunoSettings::getInstance().isShadowBanned(realId);
	const auto toggleShadowBan = [=]
	{
		if (shadowBanned) {
			YunoSettings::getInstance().removeShadowBan(realId);
		} else {
			YunoSettings::getInstance().addShadowBan(realId);
		}
	};

	addCallback({
		.text = (shadowBanned
					 ? tr::yuno_FiltersQuickUnshadowBan(tr::now)
					 : tr::yuno_FiltersQuickShadowBan(tr::now)),
...
void AddDeleteOwnMessagesAction(PeerData *peerData,
	...
	addCallback(
		tr::yuno_DeleteOwnMessages(tr::now),
...
void AddHistoryAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto has = YunoMessages::hasRevisions(item);
	if (!has) {
		return;
	}

	menu->addAction(
		tr::yuno_EditsHistoryMenuText(tr::now),
		[=]
		{
...
void AddHideMessageAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::yuno_ContextHideMessage(tr::now),
		[=]()
		{
...
void AddUserMessagesAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	if (item->history()->peer->isChat() || item->history()->peer->isMegagroup()) {
		menu->addAction(
			tr::yuno_UserMessagesMenuText(tr::now),
			[=]
			{
...
void AddMessageDetailsAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	callback(Window::PeerMenuCallback::Args{
		.text = tr::yuno_MessageDetailsPC(tr::now),
		.handler = nullptr,
		...
		.fillSubmenu = [&](not_null<Ui::PopupMenu*> menu2)
		{
			if (hasAnyPostField) {
				if (!messageViews.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::yuno_MessageDetailsViewsPC(tr::now),
						messageViews
					));
				}
				...
				if (!messageForwards.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						...
						tr::yuno_MessageDetailsSharesPC(tr::now),
						messageForwards
					));
				}
			...
			menu2->addAction(Ui::ContextActionWithSubText(
				menu2->menu(),
				st::menuIconSchedule,
				tr::yuno_MessageDetailsDatePC(tr::now),
			...
			if (view && view->displayedEditDate()) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconEdit,
					tr::yuno_MessageDetailsEditedDatePC(tr::now),
			...
			if (isForwarded) {
				menu2->addAction(Ui::ContextActionWithSubText(
					menu2->menu(),
					st::menuIconTTL,
					tr::yuno_MessageDetailsForwardedDatePC(tr::now),
			...
			if (media && hasAnyMediaField) {
				...
				if (!mediaSize.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconDownload,
						tr::yuno_MessageDetailsFileSizePC(tr::now),
						mediaSize
					));
				}
				...
				if (!mediaMime.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconShowAll,
						tr::yuno_MessageDetailsMimeTypePC(tr::now),
						mime.name()
					));
				}
				...
				if (!mediaName.isEmpty()) {
					...
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::yunoEditsHistoryIcon,
						tr::yuno_MessageDetailsFileNamePC(tr::now),
						shortified,
						[=]
						{
				...
				if (!mediaResolution.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconStats,
						tr::yuno_MessageDetailsResolutionPC(tr::now),
						mediaResolution
					));
				}
				...
				if (!mediaDC.isEmpty()) {
					menu2->addAction(Ui::ContextActionWithSubText(
						menu2->menu(),
						st::menuIconBoosts,
						tr::yuno_MessageDetailsDatacenterPC(tr::now),
						mediaDC
					));
				}
	...
	menu->addAction(
		...
		[=]
		{
			...
			if (useNoQuote) {
				...
				if (YunoForward::isFullYunoForwardNeeded(currentItem)) {
					crl::async([=]
					{
						YunoForward::forwardMessages(session, action, false, resolvedDraft);
					});
				} else if (YunoForward::isYunoForwardNeeded(currentItem)) {
					crl::async([=]
					{
						YunoForward::intelligentForward(session, action, resolvedDraft);
					});
				} else {
...
void AddReadUntilAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	const auto &ghost = YunoSettings::ghost(&item->history()->session());
	if (ghost.sendReadMessages()) {
		return;
	}

	menu->addAction(
		tr::yuno_ReadUntilMenuText(tr::now),
		[=]
		{
			readHistory(item);
...
void AddBurnAction(not_null<Ui::PopupMenu*> menu, HistoryItem *item) {
	...
	menu->addAction(
		tr::yuno_ExpireMediaContextMenuText(tr::now),
		[=]
		{
			const auto ids = MTP_vector<MTPint>(1, MTP_int(item->id));
...
void AddCreateFilterAction(not_null<Ui::PopupMenu*> menu,
	...
	menu->addAction(
		tr::yuno_RegexFilterQuickAdd(tr::now),
		[=]
		{
			RegexFilter filter;
