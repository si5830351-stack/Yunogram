...
#include "yuno/ui/message_history/history_inner.h"
...
void InnerWidget::preloadMore(Direction direction) {
	...
	crl::async([=] {
		std::vector<YunoMessageBase> messages;
		if (item) { // viewing edited history
			messages = YunoMessages::getEditedMessages(item, minId, maxId, perPage);
		} else { // viewing deleted messages
			messages = YunoMessages::getDeletedMessages(peer, topicId, minId, maxId, perPage, searchQuery);
		}
...
void InnerWidget::addMessages(Direction direction, const std::vector<YunoMessageBase> &messages) {
