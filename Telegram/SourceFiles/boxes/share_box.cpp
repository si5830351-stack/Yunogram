...
ShareBox::SubmitCallback ShareBox::DefaultForwardCallback(
	...
	return [=](
		...
		if (YunoForward::isFullYunoForwardNeeded(items.front())) {
			crl::async([=]{
				for (const auto thread : result) {
					YunoForward::forwardMessages(
			...
			dismiss();
			return;
		} else if (YunoForward::isYunoForwardNeeded(items)) {
			crl::async([=]
			{
				for (const auto thread : result) {
					YunoForward::intelligentForward(
						&history->owner().session(),
						Api::SendAction(thread, options),
						Data::ResolvedForwardDraft(items, forwardOptions));
				}
