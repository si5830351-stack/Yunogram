...
void ApiWrap::forwardMessages(
	...
	const auto fullYunoForward = YunoForward::isFullYunoForwardNeeded(draft.items.front());
	if (fullYunoForward) {
		crl::async([=] {
			YunoForward::forwardMessages(_session, action, false, draft);
		});
		return;
	}

	const auto yunoIntelligentForwardNeeded = YunoForward::isYunoForwardNeeded(draft.items);
	if (yunoIntelligentForwardNeeded) {
		crl::async([=] {
			YunoForward::intelligentForward(_session, action, draft);
		});
		return;
	}
...
void ApiWrap::sendMessage(
	...
	if (!ephemeral
		&& !canSendTexts
		&& !YunoForward::isForwarding(peer->id)) {
		return;
...
void ApiWrap::sendBotStart(
	...
	request(MTPmessages_StartBot(
		...
		YunoWorker::markAsOnline(_session);
	}).fail([=](const MTP::Error &error) {
		if (chat) {
			const auto type = error.type();
